...
export class DB {
  ...
  //work in progress, adds song to sql db
  public static async insertSong(args: string[]): Promise<any> {
    //path should always be add_songs
    console.log("Database.ts -> insertSong");
    const schema = await slurp('add_songs');
    const db = await this.getDB();
    console.log("args: " + JSON.stringify(args));
    const notes = args[3].match(new RegExp(/[A-G]+\d/g))?.join(" ");
    
    console.log("notes: " + notes);
    db.all(schema, [args[0],args[1],args[2],args[3]], (err, results) => {
      console.log(err);
    }
    );
  }
