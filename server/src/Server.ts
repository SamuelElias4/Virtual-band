...
import { AddSongHandler } from './handlers/AddSongHandler';
...
export const validMessages: MessageHandler[] = [GetSongsHandler, AddSongHandler];
...
function connectHandler(socket: Socket) {
   ...
   socket.onAny((event, msg) =>
      console.debug(`Server.ts -> conenctHandler: ${event}[${socket.id}]: ${JSON.stringify(msg)}`),
   );
